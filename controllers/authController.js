...
const signToken = (id) =>
  jwt.sign(
    { id, iat: Math.floor(Date.now() / 1000) + 100 },
...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    expires: new Date(
      Date.now() +
        process.env.JWT_COOKIE_EXPIRES_IN.slice(0, -1) * 86400 * 1000,
    ),
    httpOnly: true,
  };
...
exports.forgotPassword = async (req, res, next) => {
  ...
  if (!req.body.email) {
    return next(new AppError('Please indicate your email.', 400));
  }
  ...
  if (user) {
    ...
    try {
      ...
      return next(
        new AppError(
          'There was an error sending the email. Please try it again.',
          500,
        ),
      );
    }
  ...
  res.status(200).json({
    status: 'success',
    message: 'Email has been sent if your email has been signed up.',
  });
};
